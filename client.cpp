...
Client::Client(QObject *parent)
    : QObject(parent), serverAddress(QString("127.0.0.1")), serverPort(5000),
      clientPort(5001), clientServer(clientPort, this),
      serverConnection(clientInfo, rsa, this), nextId(0) {
  if (!initialize()) {
  ...
  connect(&clientServer, SIGNAL(incomingConnectionSignal(quintptr)), this,
          SLOT(incomingConnection(quintptr)));
  connect(&serverConnection, SIGNAL(onRegistrationReply(QString, QString)),
          this, SLOT(registrationReplyReceived(QString, QString)));
  connect(&serverConnection, SIGNAL(onUpdatedActiveClients(QJsonArray)), this,
          SLOT(updatedActiveClients(QJsonArray)));
  connect(&serverConnection, SIGNAL(onRequestReceived(ClientInfo)), this,
          SLOT(receiveCreateChannelRequest(ClientInfo)));
  connect(&serverConnection,
          SIGNAL(onChannelReplyReceived(ClientInfo, bool, int)), this,
          SLOT(receiveCreateChannelReply(ClientInfo, bool, int)));

  serverConnection.connectToServer(serverAddress, serverPort);
  serverConnection.sendSymKey();
}

const QList<QString> *Client::getActiveClients() { return &activeClients; }
...
void Client::sendMessage(QString dest, QString text) {
  ...
  // open channel for client not found, which means inconsistenct between our UI
  // and client class.
...
void Client::incomingConnection(quintptr socketDescriptor) {
  ...
  qDebug() << "incoming connection by connecting to client server on port: "
           << socketDescriptor;
  // here we have to somehow verify whether the host attemptint to connect is
  // the host that we accepted the request from
  Channel *channel = new Channel(&expectingConnections, &expectingIds,
                                 socketDescriptor, true, rsa, this);
  connect(channel, SIGNAL(onMessageReceived(QString, QString)), this,
          SLOT(messageReceived(QString, QString)));
  connect(channel, SIGNAL(onChannelActive(QString)), this,
          SLOT(channelActive(QString)));
  connect(channel, SIGNAL(channelRemove(QString)), this,
          SLOT(leaveChannel(QString)));
...
void Client::channelRequestAccepted() {
  ...
  qDebug()
      << "channel request accepted by user on client for communication with: "
      << clInfo.name;
