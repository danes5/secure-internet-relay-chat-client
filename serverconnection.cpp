...
ServerConnection::ServerConnection(ClientInfo &clInfo, rsautils &rsa,
...
QByteArray ServerConnection::encryptCreateChannelReply(bool reply,
                                                       QString clientName,
                                                       int id) {
  QJsonObject jsonObject;
...
void ServerConnection::connectToServer(QString serverAddress, quint16 port) {
  ...
  // line added because ssl throws error that certificate is not verified on
  // your system,
...
void ServerConnection::connected() { qDebug() << "connected"; }
  ...
  if (buffer.fullMessageRead()) {
    ...
    if (encrypted) {
      ...
      if (type == "ret_cli") {
        ...
        qDebug() << "received request for communication from: client: "
                 << clInfo.name;
        ...
        qDebug() << "received reply for communication from: client: "
                 << clInfo.name << "  with id: " << id;
        bool acc = res == "acc" ? true : false;
        emit onChannelReplyReceived(clInfo, acc, id);
      } else if (type == "reg_rep") {
      ...
      // this means that we have received a message before a secure channel was
      // established
      qDebug() << "server connection should never receive message not "
                  "encrypted by aes";
    }
  }
  buffer.reset();
