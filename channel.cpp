...
Channel::Channel(ClientInfo otherClient, bool genKey, rsautils &rsa,
                 QObject *parent)
    : QObject(parent), otherClientInfo(otherClient), encrypted(false),
      otherSideAuthorized(false), generateSymKey(genKey), hasInfo(true),
      rsa(rsa) {
...
Channel::Channel(QList<ClientInfo> *connections, QList<int> *ids,
                 quintptr descriptor, bool genKey, rsautils &rsa,
                 QObject *parent)
    : QObject(parent), encrypted(false), otherSideAuthorized(false),
      generateSymKey(genKey), hasInfo(false), connections(connections),
      ids(ids), rsa(rsa) {
...
QString Channel::getOtherClientName() { return otherClientInfo.name; }
...
QByteArray Channel::encryptSendAuthorizarionMessage() {
...
void Channel::sendAuthorizationMessage() {
  socket->write(encryptSendAuthorizarionMessage());
...
void Channel::readyRead() {
  ...
  if (buffer.fullMessageRead()) {
    ...
    if (!hasInfo) {
      ...
      ids->removeAt(index);
      qDebug() << "channel connected and recognized client: "
...
void Channel::setkey(unsigned char *newKey) { gcm.setKey(newKey); }
