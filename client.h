...
signals:
  ...
  /**
   * @brief onRegistrationSuccessful signal emmited after registration was
   * successful
   */
  ...
  /**
   * @brief onChannelRequestReceived signal emmited when received request to
   * start communication from other client
   * @param name other client
   */
  void onChannelRequestReceived(QString name);

  /**
   * @brief onChannelActive signal emmited when channel is ready to send
   * messages
   * @param name other client
   */
...
public slots:
  ...
  /**
   * @brief updateActiveClients update active clients, called after after user
   * querried for list of active clients
   */
  void updateActiveClients();

  /**
   * @brief updatedActiveClients called after list of active clients was
  ...
  /**
   * @brief sendCreateChannelRequest called from UI, attempts to start
   * communication with other client
   * @param name other client
   */
  void sendCreateChannelRequest(QString name);

  /**
   * @brief receiveCreateChannelRequest receives request from other client and
   * raises signal for ui
   * @param name other client
   */
  void receiveCreateChannelRequest(ClientInfo clInfo);

  /**
   * @brief sendMessage attempts to send messsage to client with whom connection
  ...
  /**
   * @brief incomingConnection called after new connection was raised on the
   * client server
  ...
  /**
   * @brief messageReceived slot called after message was received, emits signal
   * for ui
  ...
  /**
   * @brief registrationReplyReceived slot called after registration reply was
   * received from server
  ...
  /**
   * @brief channelRequestAccepted slot called from ui after client accepted
  ...
  /**
   * @brief channelActive slot called after channel becomes active so user can
   * write messages
   */
  void channelActive(QString name);

  /**
   * @brief receiveCreateChannelReply receive reply from other client for
