...
QByteArray rsautils::encryptMessage(QByteArray data) {
  ...
  int res = 0;
  if (length < 129) {
    res = mbedtls_rsa_pkcs1_encrypt(&partnerPublicKey, mbedtls_ctr_drbg_random,
                                    &ctr_drbg, MBEDTLS_RSA_PUBLIC, length,
    ...
    for (i = 0; i < (length - 128); i += 128) {
      res = mbedtls_rsa_pkcs1_encrypt(
          &partnerPublicKey, mbedtls_ctr_drbg_random, &ctr_drbg,
          MBEDTLS_RSA_PUBLIC, 128,
          (const unsigned char *)(data.constData() + i), output);
      if (res != 0) {
        qDebug() << "rsa encryption failed: " << res;
      }
      encrypted += QByteArray((const char *)output, 256);
...
QByteArray rsautils::decryptMessage(QByteArray data) {
  ...
  if (length < 257) {
    res = mbedtls_rsa_pkcs1_decrypt(
        &myKeys, mbedtls_ctr_drbg_random, &ctr_drbg, MBEDTLS_RSA_PRIVATE, &ii,
        (const unsigned char *)data.constData(), result, 2048);
    ...
    for (i = 0; i < (length - 256); i += 256) {
      ...
      res = mbedtls_rsa_pkcs1_decrypt(
          &myKeys, mbedtls_ctr_drbg_random, &ctr_drbg, MBEDTLS_RSA_PRIVATE, &ii,
          (const unsigned char *)(data.constData() + i), result, 2048);
      if (res != 0) {
        qDebug() << "rsa decryption failed: " << res;
      }
      decrypted += QByteArray((const char *)result, ii);
...
int rsautils::setMyKey() {
  return mbedtls_rsa_gen_key(&myKeys, mbedtls_ctr_drbg_random, &ctr_drbg,
                             KEY_SIZE, EXPONENT);
}
...
void rsautils::setPartnerPublicKey(QJsonDocument key) {
  ...
  mbedtls_mpi_read_string(&partnerPublicKey.N, 16,
                          jsonObject["N"].toString().toStdString().c_str());
  mbedtls_mpi_read_string(&partnerPublicKey.E, 16,
                          jsonObject["E"].toString().toStdString().c_str());
