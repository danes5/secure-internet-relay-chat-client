...
QByteArray rsautils::encryptMessage(QByteArray data)
{
    ...
    if(length < 129)
    {
        res = mbedtls_rsa_pkcs1_encrypt( &partnerPublicKey, mbedtls_ctr_drbg_random,  &ctr_drbg, MBEDTLS_RSA_PUBLIC, length, (const unsigned char *)data.constData(), output );
        if (res != 0){
            qDebug() << "rsa encryption failed: " << res;
        }
    ...
    else
    {
        ...
        {
            res = mbedtls_rsa_pkcs1_encrypt( &partnerPublicKey, mbedtls_ctr_drbg_random,  &ctr_drbg, MBEDTLS_RSA_PUBLIC, 128, (const unsigned char *)(data.constData()+i), output );
            if (res != 0){
                qDebug() << "rsa encryption failed: " << res;
            }
            encrypted += QByteArray((const char *)output, 256);
        ...
        res = mbedtls_rsa_pkcs1_encrypt( &partnerPublicKey, mbedtls_ctr_drbg_random,  &ctr_drbg, MBEDTLS_RSA_PUBLIC, length-i, (const unsigned char *)(data.constData()+i), output );
        if (res != 0){
            qDebug() << "rsa encryption failed: " << res;
        }
        encrypted += QByteArray((const char *)output, 256);
    ...
    quint64 len = encrypted.length();
    char cData [len + sizeof(quint64)];
    memcpy(cData, (char*)&len, sizeof(quint64));
    memcpy(cData + sizeof(quint64) ,encrypted.data(), len);
...
QByteArray rsautils::decryptMessage(QByteArray data)
{
    ...
    if(length < 257)
    {
        res = mbedtls_rsa_pkcs1_decrypt( &myKeys, mbedtls_ctr_drbg_random, &ctr_drbg, MBEDTLS_RSA_PRIVATE, &ii, (const unsigned char *)data.constData(), result, 2048 );
        decrypted = QByteArray((const char *)result, ii);
        if (res != 0){
            qDebug() << "rsa decryption failed: " << res;
        }
    }
    else
    {
        ...
        {
            ii = 256;
            res = mbedtls_rsa_pkcs1_decrypt( &myKeys, mbedtls_ctr_drbg_random, &ctr_drbg, MBEDTLS_RSA_PRIVATE, &ii, (const unsigned char *)(data.constData()+i), result, 2048 );
            if (res != 0){
                qDebug() << "rsa decryption failed: " << res;
...
void rsautils::setPartnerPublicKey(QJsonDocument key)
{
    ...
    mbedtls_mpi_read_string(&partnerPublicKey.N,16,jsonObject["N"].toString().toStdString().c_str());
    mbedtls_mpi_read_string(&partnerPublicKey.E,16,jsonObject["E"].toString().toStdString().c_str());
